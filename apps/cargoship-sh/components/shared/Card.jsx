...
Card.Description = function CardDescription({ children }) {
  return <p className="relative mt-2 text-sm  text-slate-400">{children}</p>;
};

Card.Cta = function CardCta({ children }) {
  return (
    <div aria-hidden="true" className="text-brand-light relative mt-4 flex items-center text-sm font-medium">
      {children}
...
Card.Eyebrow = function CardEyebrow({
  ...
  return (
    <Component
      className={clsx(
        className,
        "relative order-first mb-3 flex items-center text-sm text-slate-500",
        decorate && "pl-3.5"
      )}
      {...props}>
      {decorate && (
