          ...
          <div className=" mx-auto grid max-w-5xl grid-cols-1 gap-6 px-2 sm:grid-cols-2">
            {shuffledOptions.map((option) => (
              <label htmlFor={`${element.id}-${option.value}`} key={`${element.id}-${option.value}`}>
                <div className="drop-shadow-card duration-120 relative cursor-default rounded-lg border border-slate-700 bg-slate-700 p-6 transition-all ease-in-out hover:scale-105">
                  <div className="absolute right-10">
                    <input
                      ...
                      type="checkbox"
                      value={option.value}
                      className="text-brand focus:ring-brand border-brand h-5 w-5 rounded border-2 bg-slate-600"
                  ...
                  <span className="text-md mt-3 mb-1 font-bold text-slate-200">{option.label}</span>
                  <p id={`${element.id}-${option.value}-description`} className="mt-1 text-xs text-slate-400">
