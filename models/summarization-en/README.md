...
Rest-API for text summarization in english language
...
```bash
docker build -t summarization-en .
```

Run the Docker Image with:

```bash
docker run -d --rm -p 8000:80 --env API_KEY=CHANGE_ME --name summarization-en summarization-en
```
...
```bash
docker logs summarization-en
```

To stop the container run:

```bash
docker stop summarization-en
```
