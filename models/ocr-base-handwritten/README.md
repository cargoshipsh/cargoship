...
```bash
docker build -t ocr-base-handwritten .
```

Run the Docker Image with:

```bash
docker run -d --rm -p 8000:80 --env API_KEY=CHANGE_ME --name ocr-base-handwritten ocr-base-handwritten
```
...
```bash
docker logs ocr-base-handwritten
```

To stop the container run:

```bash
docker stop ocr-base-handwritten
```
