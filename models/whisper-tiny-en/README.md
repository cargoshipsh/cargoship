...
```bash
docker build -t whisper-tiny-en .
```

Run the Docker Image with:

```bash
docker run -d --rm -p 8000:80 --env API_KEY=CHANGE_ME --name whisper-tiny-en whisper-tiny-en
```
...
```bash
docker logs whisper-tiny-en
```

To stop the container run:

```bash
docker stop whisper-tiny-en
```
