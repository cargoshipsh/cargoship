...
```bash
docker build -t sentiment-de .
```

Run the Docker Image with:

```bash
docker run -d --rm -p 8000:80 --env API_KEY=CHANGE_ME --name sentiment-de sentiment-de
```
...
```bash
docker logs sentiment-de
```

To stop the container run:

```bash
docker stop sentiment-de
```
